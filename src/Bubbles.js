...
export default function Bubbles(props) {
  ...
  const Sketch = (p5) => {
    p5.setup = () => {
      ...
      let myCanvas = p5.createCanvas(600, 250);
    ...
    p5.draw = () => {
      if (p5.key == " ") {
        ...
        if (futureTime - currentTime > 1500) {
          p5.background(255);
          for (let i = 0; i < 72; i++) {
            p5.stroke(255, 0, 0, 249.9);
            totalArray[i].show2();
          ...
          p5.fill(0);
          p5.strokeWeight(0);
          let label1 = p5.text("Jewish: 15M", p5.width / 1.37, 230);
          let label2 = p5.text("Anti-Semitic: 1.09B", p5.width / 9, 230);
          let label3 = p5.text("Non-Anti-Semitic: 3B", p5.width / 2.4, 230);
          p5.pop();
        }
      }
    };
